...
def get_hs_stock_symbols() -> list:
    ...
    if _HS_SYMBOLS is None:
        ...
        # Load cache first to avoid failing on transient network issues
        if symbol_cache_path.exists():
            try:
                with symbol_cache_path.open("rb") as fp:
                    ...
                    logger.info(f"Loaded {len(cache_symbols)} symbols from cache {symbol_cache_path}")
            except Exception:
                logger.warning("Failed to load HS symbol cache, will try network")

        _retry = 60
        attempts = 0
        # Try to fetch from EastMoney, but tolerate transient errors and use cache as fallback
        while len(symbols) < MINIMUM_SYMBOLS_NUM and attempts < _retry:
            attempts += 1
            try:
                fetched = _get_symbol()
                symbols |= fetched
                logger.info(f"Fetched {len(fetched)} symbols (total {len(symbols)}) on attempt {attempts}")
            except requests.exceptions.HTTPError as e:
                logger.warning(f"EastMoney request failed on attempt {attempts}: {e}")
            except Exception as e:
                logger.warning(f"Unexpected error fetching HS symbols on attempt {attempts}: {e}")
        ...
        # If still insufficient, but we have cached symbols, use them with a warning
        if len(symbols) < MINIMUM_SYMBOLS_NUM:
            if symbol_cache_path.exists():
                try:
                    with symbol_cache_path.open("rb") as fp:
                        ...
                        symbols |= set(cache_symbols)
                        logger.warning(
                            f"Using cached HS symbols ({len(cache_symbols)}) after failed network fetches ({attempts} attempts)"
                        )
                except Exception:
                    logger.error("Failed to load HS symbol cache; symbol list may be incomplete")

        # persist cache (overwrite with new union)
        try:
            with symbol_cache_path.open("wb") as fp:
                pickle.dump(sorted(list(symbols)), fp)
        except Exception:
            logger.warning(f"Cannot write HS symbol cache to {symbol_cache_path}")
