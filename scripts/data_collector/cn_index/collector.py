...
try:
    ...
    _logger.warning("optional dependency 'baostock' not installed; some index collectors will be disabled. Install with: pip install baostock")
...
class CSI500Index(CSIIndex):
    ...
    def get_history_companies(self) -> pd.DataFrame:
        ...
        if bs is None:
            raise ImportError(
                "baostock is required for CSI500 history collection. Install with: pip install baostock"
            )
        bs.login()
    ...
    def get_data_from_baostock(date) -> pd.DataFrame:
        ...
        """
        if bs is None:
            raise ImportError("baostock is required for get_data_from_baostock. Install with: pip install baostock")
    ...
    def get_new_companies(self) -> pd.DataFrame:
        ...
        if bs is None:
            raise ImportError("baostock is required for CSI500 new companies. Install with: pip install baostock")
